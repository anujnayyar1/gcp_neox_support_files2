...
## To add to gpt-neox/megatron/text_generation_utils.py  if it gives option of putting in listener mode



### Adam's mode to make a listener. 
### generate_samples_from_prompt() might do exactly this, so not sure necessary
### Still need to edit this to make it listen; also not sure it takes a prompt
def generate_samples_listener(
    neox_args,
    model,
    maximum_tokens: int = 64,
    eos_token_id: int = None,
    ...
    while True:      # endless loop
    	...
    	## Flask won't need the endless loop
        model.module.clear_cache()  # clear kv cache between batches
        torch.distributed.barrier(group=mpu.get_model_parallel_group()) # synchronises all backprocesses
            # mpu.get_model_parallel_group()  get parallel group process belongs to
        terminate_runs = 0

        # is initialised: Checking if the default process group has been initialized
        # torch.distributed.get_rank: Returns the rank of the current process in the provided group (no group provided by default, so global maybe)
        # torch.distributed.get_world_size(): returns number of processes in given group. Global maybe by default as group=None
        if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
            os.system("clear")   # make screen blank
            raw_text = input("Context prompt >>> ")  # get input
            context_tokens = neox_args.tokenizer.tokenize(raw_text)  # tokenise input raw_text
            if len(context_tokens) == 0:
                context_tokens = [neox_args.tokenizer.eod]   # probably no action, perhaps going to next iter in loop
            context_length = len(context_tokens)
            if context_length >= (neox_args.seq_length - 1):  # prevent adding too many tokens
                print_rank_0(
                    "\nContext length"
                    + str(context_length)
                    + "\nReached max sequence length!"
        ...
        ### do soemthing if torch isnt initialised
        else:
            context_tokens = neox_args.tokenizer.tokenize("EMPTY TEXT")
            context_length = len(context_tokens)
        ...
        for (
            ...
            batch_token_generation_end_index,
            is_done,
        ) in stream_tokens(     ## iterator producing text completions, inc model info
            neox_args=neox_args,
            model=model,
            context_tokens=[context_tokens],
            eos_token_id=eos_token_id,
